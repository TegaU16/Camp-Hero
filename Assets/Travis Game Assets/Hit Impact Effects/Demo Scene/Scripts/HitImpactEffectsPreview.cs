	...
	public class HitImpactEffectsPreview : MonoBehaviour
	{
		...
		void Update()
		{
			...
			if(Input.GetMouseButtonDown(0))
			{
				...
				{
                    if (floorCollider.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out RaycastHit hit, 1000f))
                    {
			...
			if(Input.GetMouseButtonDown(1))
			{
				...
				Camera.main.transform.SetPositionAndRotation(initCamPosition, initCamRotation);
                if (cameraRotating)
				{
					ToggleRotation();
				}
